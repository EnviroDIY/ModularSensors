...
#include "ms_cfg.h" //must be before ms_common.h
//#include "ms_common.h"
...
const long serialBaud  = CONFIG_serialBaud;   // Baud rate for the primary serial port for debugging
const int8_t greenLED  = CONFIG_greenLED; // MCU pin for the green LED (-1 if not applicable)
const int8_t redLED    = CONFIG_redLED;   // MCU pin for the red LED (-1 if not applicable)
const int8_t buttonPin = CONFIG_buttonPin;// MCU pin for a button to use to enter debugging mode  (-1 if not applicable)
const int8_t wakePin   = CONFIG_wakePin;  // MCU interrupt/alarm pin to wake from sleep
// Set the wake pin to -1 if you do not want the main processor to sleep.
// In a SAMD system where you are using the built-in rtc, set wakePin to 1
const int8_t sdCardPin =  CONFIG_sdCardPin;         // MCU SD card chip select/slave select pin (must be given!)
const int8_t sensorPowerPin = CONFIG_sensorPowerPin;// MCU pin controlling main sensor power (-1 if not applicable)
...
const long ModemBaud         = CONFIG_ModemBaud;      // Communication speed of the modem
const int8_t modemVccPin     = CONFIG_modemVccPin;    // MCU pin controlling modem power (-1 if not applicable)
const int8_t modemSleepRqPin = CONFIG_modemSleepRqPin;// MCU pin used for modem sleep/wake request (-1 if not applicable)
const int8_t modemStatusPin  = CONFIG_modemStatusPin; // MCU pin used to read modem status (-1 if not applicable)
const bool modemStatusLevel  = CONFIG_modemStatusLevel;//LOW;  // The level of the status pin when the module is active (HIGH or LOW)
