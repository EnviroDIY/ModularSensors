...
class Sensor {
 public:
    ...
    // constructor values This gets the place the sensor is installed ON THE
    // ARDUINO (ie, pin number)
    ...
    // These functions get and set the number of readings to average for a
    // sensor Generally these values should be set in the constructor
    ...
    // This wakes the sensor up, if necessary - that is, does whatever it takes
    // to get a sensor in the proper state to begin a measurement after the
    // power is on. This *may* require a waitForWarmUp() before wake commands
    // can be sent. The wait is NOT included in this function! This also sets
    // the _millisSensorActivated timestamp. By default, verifies the power is
    // on and returns true
    ...
    // measurements will be any good. The waits are NOT included in this
    // function!
    ...
    // will be any good. This will often require a
    // waitForMeasurementCompletion() to ensure a measurement is done. The waits
    ...
    void clearValues();
    // This verifies that a measurement is OK (ie, not -9999) before adding it
    // to the array
    ...
    // between the sensor being awoken/activated and being ready to output
    // stable values.  The "waitForStability()" function delays until the time
    // passes.
