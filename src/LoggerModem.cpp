...
void loggerModem::modemPowerDown(void) {
  if (_powerPin >= 0) {
    // loggerModem::_priorPoweredDuration = ((float)(millis() - _millisPowerOn))
    // / 1000; MS_DBG(F("Total modem power-on time (s):"),
...
bool loggerModem::modemSetup(void) {
  ...
  if (_wakePulse_ms > 0 && _statusPin >= 0) {
    // If there's a pulse wake up (ie, non-zero wake time) and there's a status
    // pin, use that to determine if the modem was awake before setup began.
    wasAwake = digitalRead(_statusPin) == _statusLevel;
  } else if (_wakePulse_ms == 0) {
    // If the wake up is one where a pin is held (0 wake time) then we're going
    // to check the level of the held pin as the indication of whether attempts
    // were made to wake the modem before entering the setup function
    ...
    MS_DBG(F("Current state of sleep request pin"), _modemSleepRqPin, '=',
           currentRqPinState ? F("HIGH") : F("LOW"), F("meaning it should be"),
  ...
  if (!wasPowered) { // Run the sleep and power down functions
    ...
    MS_DBG(F("Because the modem was asleep prior to setup, putting it back to "
             "sleep now."));
    success &= modemSleep();
  } else {
...
bool loggerModem::modemSleep(void) {
  ...
  // NOTE:  It's possible that the modem could still be in the process of
  // turning on and thus status pin isn't valid yet.  In that case, we wouldn't
  // yet know it's coming on and so we'd mistakenly assume it's already off and
  // not turn it back off.
  // This only applies to modules with a pulse wake (ie, non-zero wake time).
  // For all modules that do pulse on, where possible I've selected a pulse time
  // that is sufficient to wake but not quite long enough to put it to sleep and
  // am using AT commands to sleep.  This *should* keep everything lined up.
...
bool loggerModem::modemSleepPowerDown(void) {
  ...
  // Now power down
  if (_powerPin >= 0) {
    // If there's a status pin available, wait until modem shows it's ready to
    // be powered off This allows the modem to shut down gracefully.
    if (_statusPin >= 0) {
...
void loggerModem::setModemPinModes(void) {
  // Set-up pin modes
  if (!_pinModesSet) {
    // NOTE:  We're going to set the power pin mode every time in power up, just
    // to be safe
    if (_statusPin >= 0) {
...
uint32_t loggerModem::parseNISTBytes(byte nistBytes[4]) {
  ...
  for (uint8_t i = 0; i < 4; i++) {
    MS_DBG(F("Response Byte"), i, '=', nistBytes[i]);
...
void loggerModem::setModemTiming(void)
{
    ...
    {
        MS_DBG(F("Resetting warm-up and disconnect timing for a u-blox
SARA-G3")); _wakeDelayTime_ms = 0;  // Module turns on when power is applied -
level of PWR_ON then irrelevant _statusTime_ms = 35;  // Time after end pulse
until V_INT becomes active
                              // Unspecified in documentation! Taking value from
Lisa U2 _max_atresponse_time_ms = 5000;  // Time until system and digital pins
are operational (5 sec typical)
        // _on_pull_down_ms = 6;  // >5ms
        // _off_pull_down_ms = 1100;  // >1s
        _disconnetTime_ms = 15000;  // Power down time "can largely vary
depending
        // on the application / network settings and the concurrent module
        // activities."  Vint/status pin should be monitored and power not
withdrawn
        // until that pin reads low.  Giving 15sec here in case it is not
monitored.
    }
    if (_modemName.indexOf(F("LISA-U2")) >= 0)
    {
        MS_DBG(F("Resetting warm-up and disconnect timing for a u-blox
LISA-U2")); _wakeDelayTime_ms = 0;  // Module turns on when power is applied -
level of PWR_ON then irrelevant _statusTime_ms = 35;  // Time after end pulse
until V_INT becomes active <35ms _max_atresponse_time_ms = 3000;  // Time until
system and digital pins are operational (3 sec typical)
        ...
        _modemName.indexOf(F("Telit LE866")) >= 0)
    {
        ...
        _max_atresponse_time_ms = 25000;  // Documentation says to wait up to 25
(!!) seconds.
        // _on_pull_down_ms = 0;  // N/A - standard chip cannot be powered on
with pin
        // _off_pull_down_ms = 0;  // N/A - standard chip cannot be powered down
with pin _disconnetTime_ms = 10000L;  // Wait with 10s time-out for sleep
    }
    if (_modemName.indexOf(F("Neoway M590")) >= 0)
    {
        MS_DBG(F("Resetting warm-up and disconnect timing for a Neoway M590"));
        _wakeDelayTime_ms = 300;  // ON/OFF pin can be held low when power is
applied
        ...
        _statusTime_ms = 300;  // Time after end pulse until VCCIO becomes
active _max_atresponse_time_ms = 300;  // Time until UART is active (300ms)
...
becomes valid - < 535 µs _statusTime_ms = 1;  // Time after VBAT is stable
before RESET becomes valid - < 535 µs _max_atresponse_time_ms = 5000;  // ??
Time to UART availability not documented
        // _on_pull_down_ms = 0;  // N/A - standard chip cannot be powered on
with pin
        // _off_pull_down_ms = 0;  // N/A - standard chip cannot be powered down
with pin
        // use AT+CPSMS command for LTE-M power saving - no other power save
method _disconnetTime_ms = 0;  // N/A - If the reset pin is used as a status
pin, it will not ever turn off
    ...
    if (_modemName.indexOf(F("Quectel M95")) >= 0)
    {
        ...
        _wakeDelayTime_ms = 30;  // Time after VBAT is stable before PWRKEY can
be used _statusTime_ms = 0;  // Time after end pulse until status pin becomes
active (54ms after start of pulse) _max_atresponse_time_ms = 500;  // UART
should respond as soon as PWRKEY pulse ends
        // _on_pull_down_ms = 2000;  // until either status key goes on, or
> 1.0 sec (~2s)
    ...
    if (_modemName.indexOf(F("Quectel MC60")) >= 0)
    {
        ...
        _wakeDelayTime_ms = 100;  // Time after VBAT is stable before PWRKEY can
be used _statusTime_ms = 0;  // Time after end pulse until status pin becomes
active (54ms after start of pulse) _max_atresponse_time_ms = 500;  // UART
should respond as soon as PWRKEY pulse ends
