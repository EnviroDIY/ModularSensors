...
#define MAX_TX_BUF_SZ 750 // DataPublisher
  ...
  if (_outClient->connect(Smtp2goJsonHost, Smtp2goJsonPort)) {
    ...
    // For ram limited systems, before adding each line/chunk to the outgoing
    // buffer, we make sure there is space for that line, sending out buffer if
    // not
    if (bufferFree() < 28)
