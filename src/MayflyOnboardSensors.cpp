...
MayflyOnboardTemp::MayflyOnboardTemp(char const *version)
  : Sensor(-1, -1, F("EnviroDIYMayflyRTC"), F("temperatureDatalogger"), F("degreeCelsius"), 2, F("BoardTemp"))
{ _version = version; }
...
MayflyOnboardBatt::MayflyOnboardBatt(char const *version)
  : Sensor(-1, -1, F("EnviroDIYMayflyBatt"), F("batteryVoltage"), F("Volt"), 3, F("Battery"))
{
    _version = version;
...
MayflyFreeRam::MayflyFreeRam(void)
  : Sensor(-1, -1, F("EnviroDIYMayflyHeap"), F("Free SRAM"), F("Bit"), 0, F("FreeRam"))
{}
