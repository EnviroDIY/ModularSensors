...
int32_t Logger::sd1_card_write_cb(uint32_t lba, uint8_t *buffer,
  ...
  return sd1_card_phy.card()->writeBlocks(lba, buffer, bufsize / 512) ? bufsize
                                                                      : -1;
}
...
#if defined USE_USB_MSC_SD0
  usb_msc.setCapacity(
      0, sdq_flashspi_phy.pageSize() * sdq_flashspi_phy.numPages() / 512, 512);
...
#ifdef USE_MS_SD_INI
void Logger::setPs_cache(persistent_store_t *ps_ram) { ps_cache = ps_ram; }
...
USE_RTCLIB *Logger::rtcExtPhyObj() { return &rtcExtPhy; }
#endif // USE_RTCLIB
