    ...
    for (unsigned long start = millis(); millis() - start < 300000; ++loops) {
      ...
      if ((0 == status) || (0x23 == status)) {
        ...
        if (10 == loops) {
          ...
          However throwing this in, might do something or maybe just coincedence
          that it started working after this
          */
...
#define TIME_PROTOCOL_PORT 37
    ...
    /* Uses "TIME" protocol on port 37 NIST: This protocol is expensive, since
      it uses the complete tcp machinery to transmit only 32 bits of data.
      FUTURE Users are *strongly* encouraged to upgrade to the network time
      protocol (NTP), which is both more accurate and more robust.*/
...
uint32_t DigiXBeeCellularTransparent::getTimeCellTower(void) {
  ...
  String res;
  uint8_t rxCellTime = 0; // 0 no Time, 1 ASCII ISO8601 time, 2 32bit Time since
                          // 2000-01-01 00:00:00
