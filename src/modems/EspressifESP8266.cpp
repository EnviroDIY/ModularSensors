...
bool EspressifESP8266::modemSleepFxn(void) {
  ...
  // sleep but no other MCU pin connected to the reset pin. NOTE:  This will NOT
  // work nicely with "testingMode"
  ...
  // Use this if you don't have access to the ESP8266's reset pin for deep sleep
  // but you do have access to another GPIO pin for light sleep.  This also sets
  // up another pin to view the sleep status.
  ...
  // - if it is still in the wakeup status, the EP8266 will enter Modem-sleep
  // mode instead;
  // - if it is NOT in the wakeup status, the ESP8266 will enter Light-sleep
  // mode.
