...
Sensor::Sensor(const char *sensorName, const uint8_t numReturnedVars,
  ...
  // addSingleMeasurementResult() function. The "waitForMeasurementCompletion()"
  // function verifies that enough time has passed.
...
bool Sensor::setup(void) {
  ...
  if (_powerPin >= 0)
    pinMode(_powerPin, OUTPUT); // NOTE:  Not setting value
  if (_dataPin >= 0)
  ...
  for (uint8_t i = 0; i < _numReturnedVars; i++) {
    if (variables[i] != NULL) // Bad things happen if try to update nullptr
    {
      MS_DBG(F("Sending value update from"), getSensorNameAndLocation(),
             F("to variable"), i, F("which is"), variables[i]->getVarName(),
             F("..."));
...
void Sensor::verifyAndAddMeasurementResult(uint8_t resultNumber,
  ...
  // If the new result is bad and there were already good results, do nothing
    ...
    MS_DBG(F("Ignoring bad result for variable"), resultNumber, F("from"),
           getSensorNameAndLocation(), F("; good results already in array."));
  }
}
void Sensor::verifyAndAddMeasurementResult(uint8_t resultNumber,
                                           int16_t resultValue) {
...
bool Sensor::checkPowerOn(bool debug) {
  ...
  if (_powerPin >= 0) {
...
#endif     // HwFeatherWing_B031ALL
    {
      ...
      powerPinOff = (bitRead(*portInputRegister(digitalPinToPort(_powerPin)),
                             powerBitNumber) == LOW);
    }
    if (powerPinOff) {
      if (debug) {
...
bool Sensor::isStable(bool debug) {
  ...
  // If the sensor has been activated but the time hasn't passed, we still need
  // to wait
  else
    return false;
}
...
bool Sensor::isMeasurementComplete(bool debug) {
  ...
  // If the sensor is measuring but the time hasn't passed, we still need to
  // wait
  else
    return false;
}
