 ...
 public:
    ...
    InsituLevelTroll(byte modbusAddress, Stream* stream, int8_t powerPin,
        ...
        : KellerParent(modbusAddress, stream, powerPin, powerPin2, enablePin,
                       measurementsToAverage, Leveltroll_InsituModel,
                       "InsituLevelTroll", KELLER_NUM_VARIABLES,
                       LEVELTROLL_WARM_UP_TIME_MS,
                       LEVELTROLL_STABILIZATION_TIME_MS,
                       LEVELTROLL_MEASUREMENT_TIME_MS) {}
    InsituLevelTroll(byte modbusAddress, Stream& stream, int8_t powerPin,
                     int8_t powerPin2 = -1, int8_t enablePin = -1,
                     uint8_t measurementsToAverage = 1)
        : KellerParent(modbusAddress, stream, powerPin, powerPin2, enablePin,
