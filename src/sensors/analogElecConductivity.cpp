...
// analogReference(ProcAdcDef_Reference); //VDDANA = 3V3
  ...
  sensorEC_adc =
      analogRead(analogPinNum); // This is not a mistake, First reading will be
                                // low beause if charged a capacitor
  ...
  /*Assuming sensorEC_adc is ratio metric - adc Reference is same as applied to
   * EC sensor. The Vcc ~ 3.3V can vary, as battery level gets low, so would be
   * nice to elliminate it in the calcs
   *
   ...
   */
  if (0 == sensorEC_adc)
    sensorEC_adc = 1; // Prevent underflow, can never be EC_SENSOR_ADC_RANGE

  Rwater_ohms =
      Rseries_ohms / (((float)EC_SENSOR_ADC_RANGE / (float)sensorEC_adc) - 1);
  ...
  if (NULL != _ptrWaterTemperature_C) {
    ...
    EC25_uScm =
        EC_uScm / (1 + TemperatureCoef * (*_ptrWaterTemperature_C - 25.0));
  } else {
    EC25_uScm = EC_uScm;
  }
