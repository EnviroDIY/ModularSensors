...
#define threshold_store(p1)
  ...
  if (newBattReading) {
    MS_DBG(F(" isBatteryStatusAbove rsp:"), retValue, F(" (Vnew="), LiIonBatt_V,
           F("V) req:"), status_req, F(" (above "), threshold_V, F("V)"));
  } else {
    MS_DBG(F(" isBatteryStatusAbove rsp:"), retValue, F(" (Vold="), LiIonBatt_V,
           F("V) req:"), status_req, F(" (above "), threshold_V, F("V)"));
  }
  return retValue;
}
...
#define CONST_VBATT_0_5BA 0.0151732
    *sensorValue_battery =
        CONST_VBATT_0_5BA * ((float)(rawBattery_adc / SAMPLE_BATTERY_PIN_NUM));
  } else if (strcmp(_version, "v0.5") == 0 or strcmp(_version, "v0.5b") == 0) {
