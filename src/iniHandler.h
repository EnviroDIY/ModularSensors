...
const char LIION_TYPE_pm[] EDIY_PROGMEM = "LIION_TYPE"; //FUT Supersede with BATTERY_TYPE
...
static int inihUnhandledFn( const char* section, const char* name, const char* value)
{
    ...
    if (strcmp_P(section,COMMON_pm)== 0) {// [COMMON] processing
        ...
        if (strcmp_P(name,LOGGER_ID_pm)== 0) {
            ...
            if ((batteryType < PSLR_NUM) && (batteryType>0) &&(errno!=ERANGE) ) {
         ...
         } else if (strcmp_P(name,LIION_TYPE_pm)== 0){ //Supersed by BATTERY_TYPE if exists
            //convert  str to num with error checking
...
const char SD_INIT_ID_pm[] EDIY_PROGMEM = "SD_INIT_ID";
        ...
        if (strcmp_P(name,BOARD_NAME_pm)== 0) {
            ...
            if (strcmp_P(value,YES_pm) == 0 ) {
                ...
                #define epc ps_ram  
                //uint8_t crc8=Dallas_crc8((const uint8_t *)((int)&epc.hw_boot+1),((uint8_t)sizeof_hw_boot)-1 );
                //epc.hw_boot.crc16=crc8;
                uint16_t crc16=calc_crc16((const uint8_t *)((int)&epc.hw_boot+SIZE_UINT16_CRC16 ),((uint16_t)sizeof(epc.hw_boot)-SIZE_UINT16_CRC16 ) );
                epc.hw_boot.crc16 =crc16;
                EEPROM.put(EP_HW_BOOT_ADDR ,epc.hw_boot);
                MS_DBG(F("hw_boot wr"),sizeof_hw_boot , 
                F("crc16="),epc.hw_boot.crc16,
                F("sn="),(char *)epc.hw_boot.serial_num,
                F("rev="),(char *) epc.hw_boot.rev,
                F("name="),(char *)epc.hw_boot.board_name 
    ...
    } else if (strcmp_P(section,USER_pm)== 0) 
    {
        if (strcmp_P(name,ACTION_pm)== 0) {
            ...
            if (strcmp_P(value,WRITE_pm) == 0 ) {
                ...
                MS_DBG(F("app eeprom wr"),sizeof(epc), 
                F("crc="),epc.app.crc16
...
#if defined USE_PS_EEPROM 
/* ==========================================================================
AVR mega1284 has a 4Kbytes eeprom - electricaly eraseable programable read only memory 
This is used to store core provisioning setup, so each processor board can be customized.

Read the eeprom into the local ram buffer. If crc is invalid then initialize to defaults.
The eeprom has a crc16 in two sections.
The epc.hw_boot_t and epc.app... 
*/
void readAvrEeprom() {
    uint16_t crc16;
    //Read EEPROM in sections, and verify CRC

    MS_DBG(F("Eeprom  size="),EEPROM.length(),"boot=",sizeof_hw_boot, "app=",sizeof(epc.app));

    EEPROM.get(EP_HW_BOOT_ADDR,epc.hw_boot);
    crc16=calc_crc16((const uint8_t *)((int)&epc.hw_boot+SIZE_UINT16_CRC16 ),((uint16_t)sizeof(epc.hw_boot)-SIZE_UINT16_CRC16 ) );

    MS_DBG(F("Eeprom  boot "),  F("crc16calc"),crc16,F("crc16read"),epc.hw_boot.crc16 );
    if ((crc16 != epc.hw_boot.crc16) && (crc16!=0)) {
        ...
        strcpy_P((char *)epc.hw_boot.board_name,(const char *)F("enviroDIY mayfly"));
        strcpy_P((char *)epc.hw_boot.serial_num,(const char *)F("M123456789abcdefgh"));
        strcpy_P((char *)epc.hw_boot.rev,(const char *)F("0.5b"));
        //strcpy_P((char *)epc.hw_boot.sd_boot_ini,(const char *)F("ms_cfg.ini"));
    } 

    PRINTOUT(F("From eeprom boot Name["), (char *)epc.hw_boot.board_name
        ,F("]  rev["), (char *) epc.hw_boot.rev
        ,F("] sn["),(char *)epc.hw_boot.serial_num,F("]"));

    MS_DBG(F("hw_boot ver"),epc.hw_boot.struct_ver);

    EEPROM.get(EP_PERSISTENT_STORE_ADDR,epc.app);
    crc16= calc_crc16((const uint8_t *)((int)&epc.app+SIZE_UINT16_CRC16 ),(sizeof(epc.app)-SIZE_UINT16_CRC16 ) );
