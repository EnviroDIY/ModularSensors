...
void ramAvailable() {
  ...
  uint16_t top_stack =
      (int)&top_stack - (__brkval == 0 ? (int)&__heap_start : (int)__brkval);
  ...
  if (strcmp_P(section, PROVIDER_pm) == 0) {
    ...
    */
    ...
    do {
      ...
      if (strcmp(
              (const char *)variableList[uuid_search_i]->getVarUUID().c_str(),
    ...
    if (strcmp_P(name, LOGGER_ID_pm) == 0) {
      ...
      if ((intervalMin > 0) && (errno != ERANGE)) {
        if (intervalMin > loggingInterval_MAX_CDEF_MIN) {
          SerialStd.print(
              F("COMMON LOGGING_INTERVAL_MINUTES must be less than : "));
    ...
    if (strcmp_P(name, INA219M_MA_MULT_pm) == 0) {
      ...
      // For INA219M_V_THRESHLOW_pm expect a string with +ve number and covert
      // to float
      ...
      if ((voltThreshold > 0) && (errno != ERANGE)) {
        ...
        ina219m_phy.setCustomVoltThreshold(voltThreshold,
                                           ina219m_voltLowThresholdAlertFn);
        SerialStd.print(F("' set to '"));
        SerialStd.print(ina219m_phy.getCustomVoltThreshold());
        SerialStd.println("'");
      } else {
...
#define epc ps_ram
        ...
        uint16_t crc16 =
            calc_crc16((const uint8_t *)((int)&epc.hw_boot + SIZE_UINT16_CRC16),
                       ((uint16_t)sizeof(epc.hw_boot) - SIZE_UINT16_CRC16));
        epc.hw_boot.crc16 = crc16;
        EEPROM.put(EP_HW_BOOT_ADDR, epc.hw_boot);
        MS_DBG(F("hw_boot wr"), sizeof_hw_boot, F("crc16="), epc.hw_boot.crc16,
               F("sn="), (char *)epc.hw_boot.serial_num, F("rev="),
    ...
    if (strcmp_P(name, ACTION_pm) == 0) {
      ...
      if (strcmp_P(value, WRITE_pm) == 0) {
        ...
        uint16_t crc16 =
            calc_crc16((const uint8_t *)((int)&epc.app + SIZE_UINT16_CRC16),
