...
// Sets the logger ID
void Logger::setLoggerID(const char *loggerID) { _loggerID = loggerID; }
...
void Logger::attachModem(loggerModem &modem) { _logModem = &modem; }
...
bool Logger::syncRTC() {
  ...
  // before the NEXT logging interval - it can take the modem that long to shut
  // down
...
int8_t Logger::getLoggerTimeZone(void) { return Logger::_loggerTimeZone; }
...
int8_t Logger::getTZOffset(void) { return Logger::_loggerRTCOffset; }
...
String Logger::formatDateTime_ISO8601(DateTime &dt) {
  ...
  if (-24 <= _loggerTimeZone && _loggerTimeZone <= -10) {
    ...
    tzString =
        tzString.substring(0, 1) + '0' + tzString.substring(1, 2) + F(":00");
  } else if (_loggerTimeZone == 0) {
    tzString = 'Z';
...
void Logger::markTime(void) { Logger::markedEpochTimeTz = getNowEpochTz(); }
...
void Logger::setFileName(String &fileName) { _fileName = fileName; }
...
bool Logger::initializeSDCard(void) {
  ...
  if (_SDCardSSPin < 0) {
    ...
    if (!sd1_card_fatfs.begin(_SDCardSSPin, SPI_FULL_SPEED)) {
      ...
      PRINTOUT(F("Data will not be saved!"));
      retVal = false;
    } else // skip everything else if there's no SD card, otherwise it might
           // hang
    {
      MS_DBG(
...
// Print out the header for debugging
  ...
  // Return false if we couldn't access the file (and were not told to create
  // it)
  else {
...
void Logger::begin() {
  ...
  MS_DBG(
      F("Setting up a watch-dog timer to fire after 5 minutes of inactivity"));
