...
// Windows processing of PIO_SRC_REV drops the "" making it a variable, which
// then can't be found!!!
...
static int  loggingMultiplierTop =
    loggingMultiplier_MAX_CDEF;  // Working TOP threshold
...
const long SerialStdBaud =
    115200;  // Baud rate for the primary serial port for debugging
#if defined(ARDUINO_AVR_ENVIRODIY_MAYFLY)
// ms_cfg:#define greenLEDPin 8        // MCU pin for the green LED (-1 if not
// applicable) ms_cfg:#define redLEDPin   9        // MCU pin for the red LED (-1
// if not applicable)
#elif defined(ADAFRUIT_FEATHER_M4_EXPRESS)
//#define greenLEDPin   8       //D8 // MCU pin for the green LED (-1 if not
//applicable) ms_cfg.h:#define redLEDPin    13       //D13 // MCU pin for the
// red LED (-1 if not applicable)
#if !defined greenLEDPin
#define greenLEDPin \
  LED_BUILTIN  // Built in LED is RED. MCU pin for the green LED (-1 if not
               // applicable)
#endif  // greenLEDPin
...
Adafruit_NeoPixel neoPixelPhy(NUM_NEOPIXELS, NEOPIXEL_PIN,
                              NEO_GRB + NEO_KHZ800);
#endif  // NUM_NEOPIXELS
...
// const int8_t greenLED=LED_BUILTIN;  //MCU pin for the green LED (-1 if not
// applicable)
#define greenLEDPin \
  LED_BUILTIN         // MCU pin for the green LED (-1 if not applicable)
#define redLEDPin -1  // Doesn't exist
#else
...
#if defined(ARDUINO_AVR_ENVIRODIY_MAYFLY)
const int8_t buttonPin = -1;  // 21 Not used -MCU pin for a button to use to
                              // enter debugging mode  (-1 if not applicable)
...
// QSPI const int8_t sdCardSSPin = 10;  // PA08 MCU SD card chip select/slave
// select pin (must be given!) and has FEATHER_RTC_SD_CARD
...
const int8_t sdCardSSPin  = SD_SPI_CS_PIN_DEF;
                            ...
                            // SD on std port with SD_CS JP3-D10 PA18  RTC
                            // PCF8522+ SD
...
// In a SAMD system where you are using the built-in rtc, set wakePin to 1
const int8_t sdCardSSPin =
    4;  // PA08 MCU SD card chip select/slave select pin (must be given!)
...
const int8_t sdCardSSPin =
    4;  // PA08 MCU SD card chip select/slave select pin (must be given!)
        ...
        // SD on std port with SD_CS JP3-D10 PA18                 RTC PCF8522+
        // SD
...
// Set the wake pin to -1 if you do not want the main processor to sleep.
// In a SAMD system where you are using the built-in rtc, set wakePin to 1
const int8_t sdCardPwrPin =
    -1;  // TODO check MCU SD card power pin (-1 if not applicable)
// uSD card & SerialFlash is SCOM3
// SAMD21 MOSI/PA20 MISO/PA22 SCK/PA21
//
const int8_t sdCardSSPin = 46;  // SS_2 PA27 variant.cpp 'pin'46  uSD  card chip
                                // select/slave select pin
const int8_t sfSsPin = 43;  // SS   PA23 variant.cpp 'pin'43 serialFlash chip
                            // select/slave select pin
#else
#error Undefined SD
#endif
...
const int8_t sensorPowerPin =
    sensorPowerPin_DEF;  // MCU pin controlling main sensor power (-1 if not
                         // applicable)
...
// processor Variable *mcuBoardBatt = new ProcessorStats_Battery(&mcuBoard,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *mcuBoardAvailableRAM = new
// ProcessorStats_FreeRam(&mcuBoard, "12345678-abcd-1234-ef00-1234567890ab");
...
analogThermistor analogTherm1parent(analogThermistor_PowerPin,
                                    analogThermistor_adcPin);
#endif  //
// ==========================================================================
...
// The SAMD21/SAMD51 has 6 "SERCOM" ports, any of which can be used for UART
...
// definitions for select boards. Variant.cpp/.h must be updated for extending
// SERCOM from std arduino framework.
...
const int8_t modemVccPin = modemVccPin_DEF;  // -2 MCU pin controlling modem
                                             // power (-1 if not applicable)
const int8_t modemStatusPin =
    modemStatusPin_DEF;  // RTS 19 MCU pin used to read modem status (-1 if not
                         // applicable)
const int8_t modemResetPin =
    modemResetPin_DEF;  // MCU pin connected to modem reset pin (-1 if
                        // unconnected)
const int8_t modemSleepRqPin =
    modemSleepRqPin_DEF;  // DTR 23 MCU pin used for modem sleep/wake request
                          // (-1 if not applicable)
const int8_t modemLEDPin = redLEDPin;  // MCU pin connected an LED to show modem
...
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *modemSignalPct = new
// Modem_SignalPercent(&modem, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *modemSignalPct = new Modem_BatteryState(&modem,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *modemSignalPct = new
// Modem_BatteryPercent(&modem, "12345678-abcd-1234-ef00-1234567890ab");
// Variable *modemSignalPct = new Modem_BatteryVoltage(&modem,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *modemSignalPct = new
// Modem_Temp(&modem, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *modemSignalPct = new Modem_ActivationDuration(&modem,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// Variable *atlasCO2 = new AtlasScientificCO2_CO2(&atlasCO2,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *atlasCO2Temp = new
// AtlasScientificCO2_Temp(&atlasCO2, "12345678-abcd-1234-ef00-1234567890ab");
...
// const int8_t I2CPower = sensorPowerPin;  // Pin to switch power on and off
// (-1 if unconnected) uint8_t AtlasDOi2c_addr = 0x61;  // Default for DO is
// 0x61 (97) All Atlas sensors have different default I2C addresses, but any of
// them can be re-addressed to any 8 bit number.  If using the default address
// for any Atlas Scientific sensor, you may omit this argument.
...
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *atlasDOpct = new
// AtlasScientificDO_DOpct(&atlasDO, "12345678-abcd-1234-ef00-1234567890ab");
...
// const int8_t I2CPower = sensorPowerPin;  // Pin to switch power on and off
// (-1 if unconnected) uint8_t AtlasECi2c_addr = 0x64;  // Default for EC is
// 0x64 (100) All Atlas sensors have different default I2C addresses, but any of
// them can be re-addressed to any 8 bit number.  If using the default address
// for any Atlas Scientific sensor, you may omit this argument.
...
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *atlasTDS = new
// AtlasScientificEC_TDS(&atlasEC, "12345678-abcd-1234-ef00-1234567890ab");
// Variable *atlasSal = new AtlasScientificEC_Salinity(&atlasEC,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *atlasGrav = new
// AtlasScientificEC_SpecificGravity(&atlasEC,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// const int8_t I2CPower = sensorPowerPin;  // Pin to switch power on and off
// (-1 if unconnected) uint8_t AtlasORPi2c_addr = 0x62;  // Default for ORP is
// 0x62 (98) All Atlas sensors have different default I2C addresses, but any of
// them can be re-addressed to any 8 bit number.  If using the default address
// for any Atlas Scientific sensor, you may omit this argument.
...
// Variable *atlasORPot = new AtlasScientificORP_Potential(&atlasORP,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// const int8_t I2CPower = sensorPowerPin;  // Pin to switch power on and off
// (-1 if unconnected) uint8_t AtlaspHi2c_addr = 0x63;  // Default for pH is
// 0x63 (99) All Atlas sensors have different default I2C addresses, but any of
// them can be re-addressed to any 8 bit number.  If using the default address
// for any Atlas Scientific sensor, you may omit this argument.
...
// const int8_t I2CPower = sensorPowerPin;  // Pin to switch power on and off
// (-1 if unconnected) uint8_t AtlasRTDi2c_addr = 0x66;  // Default for RTD is
// 0x66 (102) All Atlas sensors have different default I2C addresses, but any of
// them can be re-addressed to any 8 bit number.  If using the default address
// for any Atlas Scientific sensor, you may omit this argument.
...
// Variable *dhtHumid = new AOSongDHT_Humidity(&dht,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *dhtTemp = new
// AOSongDHT_Temp(&dht, "12345678-abcd-1234-ef00-1234567890ab"); Variable *dhtHI
...
// Variable *sq212PAR = new ApogeeSQ212_PAR(&SQ212,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// Variable *bme280Humid = new BoschBME280_Humidity(&bme280,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *bme280Temp = new
// BoschBME280_Temp(&bme280, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *bme280Press = new BoschBME280_Pressure(&bme280,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *bme280Alt = new
// BoschBME280_Altitude(&bme280, "12345678-abcd-1234-ef00-1234567890ab");
...
const int8_t OBS3Power =
    sensorPowerPin;  // Pin to switch power on and off (-1 if unconnected)
const uint8_t OBS3NumberReadings = 10;
// const uint8_t ADSi2c_addr = 0x48;  // The I2C address of the ADS1115 ADC
// Campbell OBS 3+ Low Range calibration in Volts
const int8_t OBSLowADSChannel = 0;  // The ADS channel for the low range output
const float  OBSLow_A =
    0.000E+00;  // The "A" value (X^2) from the low range calibration
const float OBSLow_B =
    1.000E+00;  // The "B" value (X) from the low range calibration
const float OBSLow_C =
    0.000E+00;  // The "C" value from the low range calibration
...
// Variable *obs3TurbLow = new CampbellOBS3_Turbidity(&osb3low,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *obs3VoltLow = new
// CampbellOBS3_Voltage(&osb3low, "12345678-abcd-1234-ef00-1234567890ab");


// Campbell OBS 3+ High Range calibration in Volts
const int8_t OBSHighADSChannel =
    1;  // The ADS channel for the high range output
const float OBSHigh_A =
    0.000E+00;  // The "A" value (X^2) from the high range calibration
const float OBSHigh_B =
    1.000E+00;  // The "B" value (X) from the high range calibration
const float OBSHigh_C =
    0.000E+00;  // The "C" value from the high range calibration
...
// Variable *obs3TurbHigh = new CampbellOBS3_Turbidity(&osb3high,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *obs3VoltHigh = new
// CampbellOBS3_Voltage(&osb3high, "12345678-abcd-1234-ef00-1234567890ab");
...
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *fivetmVWC = new
// Decagon5TM_VWC(&fivetm, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *fivetmTemp = new Decagon5TM_Temp(&fivetm,
// "12345678-abcd-1234-ef00-1234567890ab");
#endif  // SENSOR_CONFIG_GENERAL
...
// const int8_t SDI12Power = sensorPowerPin;  // Pin to switch power on and off
// (-1 if unconnected) const int8_t SDI12Data = 7;  // The SDI12 data pin
...
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *ctdTemp = new
// DecagonCTD_Temp(&ctd, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *ctdDepth = new DecagonCTD_Depth(&ctd,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *es2Temp = new
// DecagonES2_Temp(&es2, "12345678-abcd-1234-ef00-1234567890ab");
...
const float  dividerGain =
    2;  //  Default 1/gain for grove voltage divider is 10x
const uint8_t ADSi2c_addr    = 0x48;  // The I2C address of the ADS1115 ADC
...
// Variable *mplPress = new MPL115A2_Pressure(&mpl115a2,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *mplTemp = new
// MPL115A2_Temp(&mpl115a2, "12345678-abcd-1234-ef00-1234567890ab");
...
// SoftwareSerial_ExtInts &sonarSerial = softSerial1;  // For software serial if
// needed
#endif
...
const int8_t Sonar1Trigger =
    -1;  // Trigger pin (a unique negative number if unconnected) (D25 = A1)
...
// Variable *sonar1Range = new MaxBotixSonar_Range(&sonar1,
// "12345678-abcd-1234-ef00-1234567890ab");


// const int8_t Sonar2Trigger = A2;  // Trigger pin (a unique negative number if
// unconnected) (D26 = A2) MaxBotixSonar sonar2(sonarSerial, SonarPower,
// Sonar2Trigger) ; Create an ultrasonic range variable pointer Variable
// *sonar2Range = new MaxBotixSonar_Range(&sonar2,
// "12345678-abcd-1234-ef00-1234567890ab");
...
DeviceAddress OneWireAddress1 = {0x28, 0xFF, 0xBD, 0xBA,
                                 0x81, 0x16, 0x03, 0x0C};
const int8_t  OneWirePower =
    sensorPowerPin;  // Pin to switch power on and off (-1 if unconnected)
const int8_t OneWireBus =
    A0;  // Pin attached to the OneWire Bus (-1 if unconnected) (D24 = A0)
...
// with an unknown address) MaximDS18 ds18(OneWirePower, OneWireBus);
...
// Variable *ms5803Press = new MeaSpecMS5803_Pressure(&ms5803,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *ms5803Temp = new
// MeaSpecMS5803_Temp(&ms5803, "12345678-abcd-1234-ef00-1234567890ab");
...
MeterTeros11 teros11(*teros11SDI12address, SDI12Power, SDI12Data,
                     teros11NumberReadings);

// Create the matric potential, volumetric water content, and temperature
// variable pointers for the Teros 11
// Variable *teros11Ea = new MeterTeros11_Ea(&teros11,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *teros11Temp = new
// MeterTeros11_Temp(&teros11, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *teros11VWC = new MeterTeros11_VWC(&teros11,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// Variable *tbi2cTips = new RainCounterI2C_Tips(&tbi2c,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *tbi2cDepth = new
// RainCounterI2C_Depth(&tbi2c, "12345678-abcd-1234-ef00-1234567890ab");
#endif  // SENSOR_CONFIG_GENERAL
...
// Variable *inaCurrent = new TIINA219_Current(&ina219,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *inaVolt = new
// TIINA219_Volt(&ina219, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *inaPower = new TIINA219_Power(&ina219,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// Create the current, voltage, and power variable objects for the Nanolevel and
// return variable-type pointers to them Use these to create variable pointers
// with names to use in multiple arrays or any calculated variables. Variable
// *inaCurrent = new TIINA219_Current(&ina219,
// "12345678-abcd-1234-efgh-1234567890ab"); Variable *inaVolt = new
// TIINA219_Volt(&ina219, "12345678-abcd-1234-efgh-1234567890ab"); NO Power
...
// Create a reference to the serial port for modbus
// Extra hardware and software serial ports are created in the "Settings for
// Additional Serial Ports" section #if defined ARDUINO_ARCH_SAMD || defined
// ATMEGA2560 HardwareSerial &modbusSerial = Serial2;  // Use hardware serial if
// possible #else AltSoftSerial &modbusSerial = altSoftSerial;  // For software
// serial if needed
// // NeoSWSerial &modbusSerial = neoSSerial1;  // For software serial if needed
// #endif
...
// int8_t max485EnablePin = -1;  // Pin connected to the RE/DE on the 485 chip
// (-1 if unconnected)
const uint8_t y504NumberReadings =
    5;  // The manufacturer recommends averaging 10 readings, but we take 5 to
...
YosemitechY504 y504(y504ModbusAddress, modbusSerial, rs485AdapterPower,
                    modbusSensorPower, max485EnablePin, y504NumberReadings);

// Create the dissolved oxygen percent, dissolved oxygen concentration, and
// temperature variable pointers for the Y504
// Variable *y504DOpct = new YosemitechY504_DOpct(&y504,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y504DOmgL = new
// YosemitechY504_DOmgL(&y504, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *y504Temp = new YosemitechY504_Temp(&y504,
// "12345678-abcd-1234-ef00-1234567890ab");
...
YosemitechY510 y510(y510ModbusAddress, modbusSerial, rs485AdapterPower,
                    modbusSensorPower, max485EnablePin, y510NumberReadings);

// Create turbidity and temperature variable pointers for the Y510
// Variable *y510Turb = new YosemitechY510_Turbidity(&y510,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y510Temp = new
// YosemitechY510_Temp(&y510, "12345678-abcd-1234-ef00-1234567890ab");
...
YosemitechY511 y511(y511ModbusAddress, modbusSerial, rs485AdapterPower,
                    modbusSensorPower, max485EnablePin, y511NumberReadings);

// Create turbidity and temperature variable pointers for the Y511
// Variable *y511Turb = new YosemitechY511_Turbidity(&y511,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y511Temp = new
// YosemitechY511_Temp(&y511, "12345678-abcd-1234-ef00-1234567890ab");
...
// int8_t max485EnablePin = -1;  // Pin connected to the RE/DE on the 485 chip
// (-1 if unconnected)
const uint8_t y514NumberReadings =
    5;  // The manufacturer recommends averaging 10 readings, but we take 5 to
        // minimize power consumption

// Create a Y514 chlorophyll sensor object
YosemitechY514 y514(y514ModbusAddress, modbusSerial, rs485AdapterPower,
                    modbusSensorPower, max485EnablePin, y514NumberReadings);

// Create chlorophyll concentration and temperature variable pointers for the
// Y514 Variable *y514Chloro = new YosemitechY514_Chlorophyll(&y514,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y514Temp = new
// YosemitechY514_Temp(&y514, "12345678-abcd-1234-ef00-1234567890ab");
...
// int8_t max485EnablePin = -1;  // Pin connected to the RE/DE on the 485 chip
// (-1 if unconnected)
const uint8_t y520NumberReadings =
    5;  // The manufacturer recommends averaging 10 readings, but we take 5 to
        // minimize power consumption

// Create a Y520 conductivity sensor object
YosemitechY520 y520(y520ModbusAddress, modbusSerial, rs485AdapterPower,
                    modbusSensorPower, max485EnablePin, y520NumberReadings);

// Create specific conductance and temperature variable pointers for the Y520
// Variable *y520Cond = new YosemitechY520_Cond(&y520,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y520Temp = new
// YosemitechY520_Temp(&y520, "12345678-abcd-1234-ef00-1234567890ab");
...
YosemitechY532 y532(y532ModbusAddress, modbusSerial, rs485AdapterPower,
                    modbusSensorPower, max485EnablePin, y532NumberReadings);

// Create pH, electrical potential, and temperature variable pointers for the
// Y532 Variable *y532Voltage = new YosemitechY532_Voltage(&y532,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y532pH = new
// YosemitechY532_pH(&y532, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *y532Temp = new YosemitechY532_Temp(&y532,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// int8_t max485EnablePin = -1;  // Pin connected to the RE/DE on the 485 chip
// (-1 if unconnected)
const uint8_t y550NumberReadings =
    5;  // The manufacturer recommends averaging 10 readings, but we take 5 to
        // minimize power consumption

// Create a Y550 conductivity sensor object
YosemitechY550 y550(y550ModbusAddress, modbusSerial, rs485AdapterPower,
                    modbusSensorPower, max485EnablePin, y550NumberReadings);

// Create COD, turbidity, and temperature variable pointers for the Y550
// Variable *y550COD = new YosemitechY550_COD(&y550,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y550Turbid = new
// YosemitechY550_Turbidity(&y550, "12345678-abcd-1234-ef00-1234567890ab");
...
// int8_t max485EnablePin = -1;  // Pin connected to the RE/DE on the 485 chip
// (-1 if unconnected)
const uint8_t y4000NumberReadings =
    5;  // The manufacturer recommends averaging 10 readings, but we take 5 to
        // minimize power consumption

// Create a Yosemitech Y4000 multi-parameter sensor object
YosemitechY4000 y4000(y4000ModbusAddress, modbusSerial, rs485AdapterPower,
                      modbusSensorPower, max485EnablePin, y4000NumberReadings);

// Create all of the variable pointers for the Y4000
// Variable *y4000DO = new YosemitechY4000_DOmgL(&y4000,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y4000Turb = new
// YosemitechY4000_Turbidity(&y4000, "12345678-abcd-1234-ef00-1234567890ab");
// Variable *y4000Cond = new YosemitechY4000_Cond(&y4000,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y4000pH = new
// YosemitechY4000_pH(&y4000, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *y4000Temp = new YosemitechY4000_Temp(&y4000,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y4000ORP = new
// YosemitechY4000_ORP(&y4000, "12345678-abcd-1234-ef00-1234567890ab"); Variable
// *y4000Chloro = new YosemitechY4000_Chlorophyll(&y4000,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *y4000BGA = new
// YosemitechY4000_BGA(&y4000, "12345678-abcd-1234-ef00-1234567890ab");
...
// Variable *dOptoDOpct = new ZebraTechDOpto_DOpct(&dopto,
// "12345678-abcd-1234-ef00-1234567890ab"); Variable *dOptoDOmgL = new
// ZebraTechDOpto_DOmgL(&dopto, "12345678-abcd-1234-ef00-1234567890ab");
// Variable *dOptoTemp = new ZebraTechDOpto_Temp(&dopto,
// "12345678-abcd-1234-ef00-1234567890ab");
...
// new analogPinEc_EC(&analogPinEc_EC, "12345678-abcd-1234-ef00-1234567890ab"),
    ...
    new Modem_ActivationDuration(&modem,
                                 "12345678-abcd-1234-ef00-1234567890ab"),
#endif  // SENSOR_CONFIG_GENERAL
    ...
    new Variable(&ina219M_A_LowFn, 2, "Min_A", "A", "Min_A_Var",
                 INA219M_A_MIN_UUID),
#endif
#if defined INA219M_A_MAX_UUID
    new Variable(&ina219M_A_HighFn, 2, "Max_A", "A", "Max_A_Var",
                 INA219M_A_MAX_UUID),
#endif
};
#if defined loggingMultiplier_MAX_CDEF
...
Logger dataLogger(LoggerID_def, loggingInterval_def_min, sdCardSSPin, wakePin,
                  &varArray);
#if defined loggingMultiplier_MAX_CDEF
// A 2 logger runs faster and raises the Nyquist sampling rate for the true
// dataLogger
...
const char* registrationToken_def =
    registrationToken_UUID;  // Device registration token
const char* samplingFeature_def =
    samplingFeature_UUID;  // Sampling feature UUID

// Create a data publisher for the EnviroDIY/WikiWatershed POST endpoint
#include <publishers/EnviroDIYPublisher.h>
// EnviroDIYPublisher EnviroDIYPOST(dataLogger, registrationToken_def,
// samplingFeature_def);
EnviroDIYPublisher EnviroDIYPOST(dataLogger, 15, 0);
// EnviroDIYPublisher EnviroDIYPOST(); //"error: request for member 'begin' in
// 'EnviroDIYPOST', which is of non-class type 'EnviroDIYPublisher()'"
...
ThingSpeakPublisher TsMqtt(dataLogger, &modemPhy.gsmClient, thingSpeakMQTTKey,
...
void greenredflash(uint8_t numFlash = 4, unsigned long timeOn_ms = 200,
                   unsigned long timeOff_ms = 200) {
...
// Set up pins for the LED's
  ...
  while (!SerialStd && (millis() < 10000)) { ledflash(100, 1); }
  ...
  // SerialStd.print(__TIMESTAMP__); //still a ASC string Tue Dec 04 19:47:20
  // 2018
...
#endif
  MS_DBG("SysCoreClock Mhz", SystemCoreClock,
         "ResetCause=", decodeResetCause(resetCause), "&", resetBackupExit,
         ":");
...
// Start the serial connection with the modem
  ...
  if (modemVccPin >= 0) {
    ...
    MS_DBG(F("ModemVccPin not used "), modemVccPin);
  }
  if (sensorPowerPin >= 0) {
    ...
    MS_DBG(F("sensorPowerPin not used "), sensorPowerPin);
  }
...
#define modemSleepRqState_DEF HIGH
    ...
    MS_DBG(F("modemSleepRqPin not used "), modemSleepRqPin);
  }
  if (modemResetPin >= 0) {
#define modemResetState_DEF HIGH
    ...
    MS_DBG(F("modemResetPin not used "), modemResetPin);
  }

  // Set the timezones for the logger/data and the RTC
  // Logging in the given time zone
  Logger::setLoggerTimeZone(timeZone);
  Logger::setRTCTimeZone(
      0);  // Only tested for UTC (UTC+0)
...
// Attach the modem and information pins to the logger
  ...
  // del &DEEP_DEBUGGING_SERIAL_OUTPUT
  // nanolevel_snsr.setDebugStream(&SerialTty);
...
void processSensors() {
  ...
  // dataLogger.setupSensorsAndFile(); !v0.21.2 see v0.19.6 replaced
...
// we will explicitly start and end the serial connection in the loop.
    ...
    if (lastReading < ina219M_A_LowReading) {
      MS_DBG(F("ina219Alow reading="), lastReading, F("lower than"),
             ina219M_A_LowReading);
      ina219M_A_LowReading = lastReading;
      readingUpdated       = true;
    }
    if (lastReading > ina219M_A_HighReading) {
      MS_DBG(F("ina219Ahigh reading="), lastReading, F("higher than"),
             ina219M_A_HighReading);
      ina219M_A_HighReading = lastReading;
      readingUpdated        = true;
    }
    if (false == readingUpdated) {
      MS_DBG(F("ina219 reading="), lastReading, F("within"),
             ina219M_A_LowReading, F("~"), ina219M_A_HighReading);
    }
...
#define CONFIG_NIST_ERR_MASK (~0x3F)
              uint32_t nistCheckRemainder = Logger::markedEpochTime %
                  CONFIG_NIST_CHECK_SECS;
              bool nistSyncNow = false;
              if (nistSyncRtc ||
                  ((nistCheckRemainder & CONFIG_NIST_ERR_MASK) == 0)) {
                nistSyncNow = true;
                PRINTOUT(F("SyncTimeCheck Atmpt "), nistSyncRtc,
                         Logger::markedEpochTime, "remainder ",
                         nistCheckRemainder, " check+-",
                         (nistCheckRemainder & CONFIG_NIST_ERR_MASK));
              } else {
                PRINTOUT(F("SyncTimeCheck NotNeeded "), Logger::markedEpochTime,
                         "remainder ", nistCheckRemainder, " check+-",
                         (nistCheckRemainder & CONFIG_NIST_ERR_MASK));
              }
              if (nistSyncNow) {
                dataLogger.watchDogTimer.resetWatchDog();
                MS_DBG(F("  atl..Running a NIST clock sync. NeedSync "),
                       nistSyncRtc);
                nistSyncRtc = true;  // Needs to run every access until sucess
                if (true == dataLogger.syncRTC()) {
                  nistSyncRtc = false;  // Sucess
                }
                // If time very different should ensure publish is accurate -
                // possibly cancel
              }
              ...
              MS_DBG(F("  No internet connection..."));
            }
          }
...
void loop() {
  ...
  // not implemented yet: if (EnviroDIYLogger.startTesting)
  ...
  if (ulPin < thisVariantNumPins) {
    ...
    */
    if (ARD_DIGITAL_EXTENSION_PINS > vextPin) {
      MS_DEEP_DBG("***digitalWrExtD ", mcpExp.getPortStr(vextPin), ulPin, "(",
                  vextPin, ")=", ulVal);
      mcpExp.setBit((peB031_bit)(vextPin), ulVal);
    } else {
      vextPin = vextPin -
          ARD_DIGITAL_EXTENSION_PINS;  // Normalize of Analog range
...
void pinModExt(uint32_t ulPin, uint32_t ulMode) {
  if (ulPin < thisVariantNumPins) {
    ...
    MS_DEEP_DBG("***pinModExt Unhandled ", mcpExp.getPortStr(mcpPin), ulPin,
                "(", mcpPin, ")=", ulMode);
...
uint8_t digitalRdExt(uint32_t ulPin) {
  ...
  if (ulPin < thisVariantNumPins) {
    ...
    MS_DEEP_DBG("***digitalRdExt ", mcpExp.getPortStr(mcpPin), ulPin, "(",
                mcpPin, ")=", pinState);
  }
  return pinState;
}
int digitalRdMir(uint32_t ulPin) {
  ...
  if (ulPin < thisVariantNumPins) {
    ...
    MS_DEEP_DBG("***digitalRdMir ", mcpExp.getPortStr(mcpPin), ulPin, "(",
                mcpPin, ")=", pinState);
  }
  return (int)pinState;
}
